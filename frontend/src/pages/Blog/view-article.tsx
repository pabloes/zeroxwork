...
// NOTE: allow-same-origin is required for real dapps (wallet connectors, localStorage, CORS).
// This reduces iframe isolation — only safe for trusted embed sources validated server-side.
// The backend enforces a whitelist (EMBED_URL_WHITELIST in blog.ts) before persisting embedUrl.
// TODO: restrict sandbox per-origin when untrusted embeds are added.
const TRUSTED_EMBED_ORIGINS = [
    'https://pabloes.github.io',
];

function getIframeSandbox(embedUrl: string): string {
                                    ...
                                    <React.Fragment key={i}>
                                        {i > 0 && (
                                            <iframe
                                                ...
                                                sandbox={getIframeSandbox(article.embedUrl)}
                                                loading="lazy"
                                            />
                                        )}
